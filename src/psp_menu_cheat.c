...
psp_display_screen_cheat_menu(void)
{
  ...
  for (cheat_id = 0; cheat_id < CV_MAX_CHEAT; cheat_id++) {
    if (cheat_id == cur_cheat) color = PSP_MENU_SEL2_COLOR;
    else                            color = PSP_MENU_TEXT_COLOR;

    CV_cheat_t* a_cheat = &CV.cv_cheat[cheat_id];

    if (a_cheat->type != CV_CHEAT_NONE) {
      char enable = (a_cheat->type == CV_CHEAT_ENABLE) ? 'X' : ' ';
...
{
  CV_cheat_t* a_cheat = &CV.cv_cheat[cur_cheat];
...
psp_cheat_menu_add_cheat()
{
  ...
  for (cheat_id = 0; cheat_id < CV_MAX_CHEAT; cheat_id++) {
    CV_cheat_t* a_cheat = &CV.cv_cheat[cheat_id];
    if (a_cheat->type == CV_CHEAT_NONE) num_free++;
  }
  if (! num_free) num_free = 1;

  int num_add = 0;
  for (addr = 0; addr < CV_CHEAT_RAM_SIZE; addr++) {
    if ((scan_prev_ram[addr] == scan_old_value) &&
        (RAM[addr]           == scan_new_value  )) {

      for (cheat_id = 0; cheat_id < CV_MAX_CHEAT; cheat_id++) {
        CV_cheat_t* a_cheat = &CV.cv_cheat[cheat_id];
        if (a_cheat->type == CV_CHEAT_NONE) break;
      }
      if (cheat_id >= CV_MAX_CHEAT) {
        cheat_id = cur_cheat;
      }
      CV_cheat_t* a_cheat = &CV.cv_cheat[cheat_id];
      memset(a_cheat, 0, sizeof(*a_cheat));
      a_cheat->type  = CV_CHEAT_ENABLE;
      a_cheat->addr  = addr;
...
psp_cheat_menu_enable_cheat()
{
  ...
  if (a_cheat->type == CV_CHEAT_ENABLE) a_cheat->type = CV_CHEAT_DISABLE;
  else                                    a_cheat->type = CV_CHEAT_ENABLE;
}
...
{
  int cheat_id;
  int cur_type = CV_CHEAT_NONE;

  for (cheat_id = 0; cheat_id < CV_MAX_CHEAT; cheat_id++) {
    CV_cheat_t* a_cheat = &CV.cv_cheat[cheat_id];
    if (a_cheat->type != CV_CHEAT_NONE) {
      if (cur_type == CV_CHEAT_NONE) {
        if (a_cheat->type == CV_CHEAT_ENABLE) cur_type = CV_CHEAT_DISABLE;
        else                                   cur_type = CV_CHEAT_ENABLE;
      }
...
{
  if (step == 1) {
    cur_cheat++; if (cur_cheat >= CV_MAX_CHEAT) cur_cheat = 0;
  } else if (step == -1) {
    cur_cheat--; if (cur_cheat < 0) cur_cheat = CV_MAX_CHEAT - 1;
  }
}
...
psp_cheat_menu_edit_list()
{
  ...
  sprintf(TmpFileName, "%s/cheat.txt", CV.cv_home_dir);
...
{
  if (scan_prev_ram == 0) {
    scan_prev_ram = (uchar *)malloc( CV_CHEAT_RAM_SIZE );
    memcpy(scan_prev_ram, RAM, CV_CHEAT_RAM_SIZE);
  }
  psp_cheat_update_match();
}
